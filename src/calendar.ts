...
// Statische Variable für das Blättern zwischen den Wochen
let currentWeekOffset = 0;

// Die 7 Farben für Montag bis Sonntag (hier frei anpassbar)
...
export function renderCalendar(
  ...
  // 1. Berechnung der Woche (Montag bis Sonntag) basierend auf currentWeekOffset
  const now = new Date();
  ...
  // Datumsanzeige für die Kopfzeile (z.B. 03.08. - 09.08.2026)
  ...
  // 2. Erzeuge Array für alle 7 Tage der aktuellen Woche
  const weekDays: Date[] = [];
  for (let i = 0; i < 7; i++) {
  ...
  return html`
    <div class="calendar-container" style="display: flex; flex-direction: column; gap: 14px;">
      ...
      ${weekDays.map((dayDate, dayIdx) => {
        ...
        // Events filtern, die an diesem Tag stattfinden
        ...
        const dynamicIcon = _renderDynamicIcon(
          ...
          true // Zeige Wochentag oben im Icon
        );

        return html`
          <div 
            class="day-bubble"
            style="border: 2px solid ${borderColor}; border-radius: 16px; padding: 14px; background: var(--ha-card-background, var(--card-background-color, #1c1c1e)); display: flex; gap: 14px; align-items: flex-start;"
          >
            <!-- Tages-Icon links -->
...
function _triggerReRender(e: Event) {
...
export function _resolveColor(
  entityId: string,
  config?: CalendarCardPlusConfig,
): string {
  const color =
    config?.calendar_colors?.[entityId] ||
    config?.calendar_icon_color ||
    "#fa3e3e";
  return _toCssColor(color);
}

export function _resolveBackgroundColor(
  entityId: string,
  config?: CalendarCardPlusConfig,
): string {
  const color =
    config?.calendar_background_colors?.[entityId] ||
...
export function _groupEventsByDate(
  events: CalendarEvent[],
...
export function _groupEventsByDateAndCalendar(
  events: CalendarEvent[],
): { date: Date; calendar: string; events: CalendarEvent[] }[] {
  const groups: {
    [key: string]: { date: Date; calendar: string; events: CalendarEvent[] };
  } = {};
...
export function _formatDuration(
  hass: HomeAssistant,
  start: Date,
  end: Date,
  isAllDay: boolean,
  ...
  if (days >= 1) {
    parts.push(
      `${days} ${localize(hass, days === 1 ? "duration_day" : "duration_days")}`,
    );
  }
  if (hours > 0) {
...
export function _formatLocalizedDuration(
  hass: HomeAssistant,
  minutes: number,
): string {
  if (minutes < 60) {
    if (minutes === 1)
      return localize(hass, "starts_in_min", "{x}", minutes.toString());
    return localize(hass, "starts_in_mins", "{x}", minutes.toString());
  }
  if (minutes < 1440) {
    const hours = Math.round(minutes / 60);
    if (hours === 1)
      return localize(hass, "starts_in_hour", "{x}", hours.toString());
    return localize(hass, "starts_in_hours", "{x}", hours.toString());
  }
  if (minutes < 43200) {
    const days = Math.round(minutes / 1440);
    if (days === 1)
      return localize(hass, "starts_in_day", "{x}", days.toString());
    return localize(hass, "starts_in_days", "{x}", days.toString());
  }
  const weeks = Math.round(minutes / 10080);
  if (weeks === 1)
    return localize(hass, "starts_in_week", "{x}", weeks.toString());
